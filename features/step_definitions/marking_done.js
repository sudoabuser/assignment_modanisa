...
When("I click on checkbox next to {string} item", async (item) => {
    ...
    await context.page.evaluate(() => document.querySelector('body > section > section > ul > li > div > label').previousElementSibling.click()
    )
    let a = await context.page.evaluate(() => document.querySelector('body > section > section > ul > li > div > label').previousElementSibling.checked
    )
    if (a == true) {
